...
class NNUE {
    ...
    int feedForward(const Accumulator &accumulator, int outputBucketIndex) const {
        const int * outputWeights = outputWeightsBuckets[outputBucketIndex];

        int outputActivation = outputBiasBuckets[outputBucketIndex];
        for (int i = 0 ; i < ACCUMULATOR_SIZE ; i++) {
            outputActivation += ClampedReLU_QuantizedAccumulator(accumulator.values[i]) * outputWeights[i];
        }
    ...
    // Loads an accumulator perspective from a Board (piece array), slow
    ...
    static NNUE * fromFile(const char * fileName) {
        ...
        file.read((char *) network->accumulatorWeights, sizeof(int) * ACCUMULATOR_SIZE*INPUT_LAYER_SIZE);
        file.read((char *) network->accumulatorBiases, sizeof(int) * ACCUMULATOR_SIZE);
        file.read((char *) network->outputWeightsBuckets, sizeof(int) * OUTPUT_BUCKETS * ACCUMULATOR_SIZE);
        file.read((char *) &network->outputBiasBuckets, sizeof(int) * OUTPUT_BUCKETS);
        file.close();
    ...
    int accumulatorWeights[INPUT_LAYER_SIZE][ACCUMULATOR_SIZE];
    int accumulatorBiases[ACCUMULATOR_SIZE];

    int outputWeightsBuckets[OUTPUT_BUCKETS][ACCUMULATOR_SIZE];
    int outputBiasBuckets[OUTPUT_BUCKETS];

    private:

    inline int ClampedReLU_QuantizedAccumulator(int x) const {
        if (x <= 0) {
            return 0;
        }
        if (x >= QUANTIZE) {
